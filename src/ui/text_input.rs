...
impl TextInput {
    pub fn new(placeholder: impl Into<SharedString>, cx: &mut Context<Self>) -> Self {
        Self {
            ...
            compact: false,
            borderless: false,
            key_context_name: "TextInput",
        }
    }

    pub fn new_compact(placeholder: impl Into<SharedString>, cx: &mut Context<Self>) -> Self {
        let mut input = Self::new(placeholder, cx);
        input.compact = true;
        input
    }

    pub fn with_key_context(mut self, key_context_name: &'static str) -> Self {
        self.key_context_name = key_context_name;
        self
    }
    ...
    fn previous_word_boundary(&self, offset: usize) -> usize {
        let leading_text = &self.content[..offset];
        let mut word_end_offset = offset;

        for (character_index, character) in leading_text.char_indices().rev() {
            if character.is_whitespace() {
                word_end_offset = character_index;
            } else {
                break;
            }
        }

        let word_text = &self.content[..word_end_offset];
        for (character_index, character) in word_text.char_indices().rev() {
            if !is_word_character(character) {
                return character_index + character.len_utf8();
            }
        }
...
impl Element for TextElement {
    ...
    fn prepaint(
        ...
        let (display_text, text_color) = if content.is_empty() {
            (input.placeholder.clone(), hsla(0., 0., 0.85, 1.0))
        } else {
            (content, style.color)
        };
...
impl Render for TextInput {
    fn render(&mut self, window: &mut Window, cx: &mut Context<Self>) -> impl IntoElement {
        ...
        let mut root = div()
            .flex()
            .key_context(self.key_context_name)
